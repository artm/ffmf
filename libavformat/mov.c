...
//#define DEBUG_SEEK
//#define DEBUG
#if defined(DEBUG) || defined(DEBUG_SEEK)
#include <stdio.h>
#include <fcntl.h>
#endif
...
#ifdef MOV_SEEK
/*
 * if seek is performed we create this entry for each sample.
 * this introduces some redundancy, that's because i don't 
 * wanna touch the rest of the code [now].
 * 
 * the simpler way to handle quicktime format would be to have 
 * a table of {offset,size} per sample for each stream + have
 * them stringed in a double-linked list ordered by offset and 
 * have them link to their streams. it is possible to build such
 * data-structure while parsing the headers. using it would have 
 * simplified both read_packet and read_seek. should i implement
 * that? [Artem Baguinski <artm@v2.nl>]
 */
typedef struct MOVSampleIndexEntry {
    int64_t offset;
    int is_key_frame; /* TODO implement */
    int s2ch, chunk, idx_in_chunk;
} MOVSampleIndexEntry;
#endif /* MOV_SEEK */
...
typedef struct MOVStreamContext {
    ...
    MOV_esds_t esds;
#ifdef MOV_SEEK
    MOVSampleIndexEntry * samples_index;
#endif
} MOVStreamContext;
...
readchunk:
#ifdef DEBUG_SEEK
    printf("reading:  0x%llx -> 0x%llx (0x%x)\n", offset, offset+size, size);
#endif
    if(size == 0x0FFFFFFF)
...
#ifdef MOV_SEEK
static void mov_stream_find_frame_offsets(MOVStreamContext * msc)
{
    int sample=0,chunk=0,s2ch,i;
    int64_t offset;

/*
 * TODO - mark key frames
 * depends on
 * TODO read stss (sync sample) atom
 * FIXME some streams [e.g. raw] will have the same sample size => no sample_size table [???]
 */
    msc->samples_index = av_malloc(sizeof(MOVSampleIndexEntry)*msc->sample_count);
    
    for (s2ch = 0; s2ch < msc->sample_to_chunk_sz; s2ch++) {
	int stop = (s2ch+1 < msc->sample_to_chunk_sz) ?
	    /* i have to decrement the "first" field, because
	     * it counts chunks starting from 1, while in chunk_offsets
	     * the first chunk is 0 
	     */
	    msc->sample_to_chunk[s2ch+1].first - 1:
	    msc->chunk_count;
	while (chunk<stop) {
	    offset = msc->chunk_offsets[chunk];
	    for (i=0; i < msc->sample_to_chunk[s2ch].count; i++) {
		msc->samples_index[sample].offset = offset;
		msc->samples_index[sample].is_key_frame = 1; /* <<< FIXME */
		msc->samples_index[sample].chunk = chunk; 
		msc->samples_index[sample].s2ch = s2ch; 
		msc->samples_index[sample].idx_in_chunk = i; 
		offset += msc->sample_sizes[sample];
		sample++;
	    }
	    chunk++;
	}
...
static int mov_read_seek(AVFormatContext *s, int stream_index, int64_t timestamp)
{
...
#ifdef DEBUG_SEEK
	printf("no appropriate stream found\n");
#endif
        return -1;
    found:
        stream_index = i;
    ...
    if (st->codec.codec_type != CODEC_TYPE_VIDEO)
        return -1;
    msc = (MOVStreamContext*)st->priv_data;
    if (msc->samples_index == NULL) {
	mov_stream_find_frame_offsets(msc);
    }

    /* compute the frame number */
    frame_number = (timestamp * st->codec.frame_rate) / 
	(st->codec.frame_rate_base * (int64_t)AV_TIME_BASE);
    if (frame_number >= msc->sample_count)
	return -1;
#ifdef DEBUG_SEEK
    printf("timestamp: %lld, frame_number: %d\n",timestamp,frame_number);
#endif
    /* find a closest key frame before */
    while (frame_number >= 0 && msc->samples_index[frame_number].is_key_frame == 0)
	frame_number--;
    if (frame_number < 0)
	return -1;
#ifdef DEBUG_SEEK
    ...
    /* find frame offset */
    offset = msc->samples_index[frame_number].offset;
#ifdef DEBUG_SEEK
    printf("offset: 0x%llx\n",offset);
#endif

    /* point the context there */
    /* TODO should point other streams as well */
    mov->next_chunk_offset = offset;
    msc->current_sample = frame_number;
    mov->partial = msc;
    msc->sample_to_chunk_index = msc->samples_index[frame_number].s2ch;
    msc->left_in_chunk = msc->sample_to_chunk[msc->sample_to_chunk_index].count - 
	msc->samples_index[frame_number].idx_in_chunk;
    msc->next_chunk = msc->samples_index[frame_number].chunk + 1;
#ifdef DEBUG_SEEK
    printf("mov->next_chunk_offset: 0x%llx\n",mov->next_chunk_offset);
    printf("msc->next_chunk: %d sample_to_chunk_index: %d current_sample: %d left_in_chunk: %d\n",
	    msc->next_chunk, msc->sample_to_chunk_index, msc->current_sample, msc->left_in_chunk);
#endif
